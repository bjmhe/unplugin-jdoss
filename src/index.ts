...
export interface Options {
  /**
   * 被上传的本地绝对路径，自行配置
   * 例如: path.resolve(workspaceRoot, "dist")
   */
  ...
  /**
   * 远程 oss 路径
   * 非必填，默认 storage.jd.local
   * 可选值:
   * - 中国: storage.jd.local
   * - 印尼: storage-local.jd.id
   * - 泰国: storage.jd.th.local
   * - 测试: test.storage.jd.com
   */
  site: string
   ...
   * 非必填
   * 1. 默认创建当前文件所在的文件夹
   * 2. 屏蔽字段或传undefined则按照localFullPath的路径一层层创建文件夹
   */
  folder: string
...
export const Starter: UnpluginInstance<Options, false> = createUnplugin(
  (options) => {
